...
**Last updated:** 2026-04-25 · **ALL ROUNDS 10-31 SHIPPED** · Rounds 32-35 staged · Last commit `938156c` · **Trilingual + creator dashboard + 7 RU profiles + SEO indexation cleanup (319 thin pages noindex'd, 14 noindex'd legacy, sitemap 517→200, 9 broken hrefs fixed)**
...
| 35 | _pending push_ | **SEO indexation cleanup masivo** — usuario mostró GSC: 567 not-indexed vs 311 indexed. Fix en 4 capas: **(1) Sitemap cleanup** · removidas 5 URLs broken (`playlist-corean` typo, `playlist-cosplay/try-on/ttl/twerk-hub` sin sufijo `-leaks`) + 1 redirect duplicate; reemplazadas con las URLs `.html` reales. Sitemap 517→200 URLs (limpieza brutal). **(2) Internal links** · auditadas 364 hrefs únicas, encontrados 9 broken. Fix: 4 RU links a páginas que no existen (`/ru/community/membership/privacy/terms`) → redirigidas a EN equivalents; 4 nav links rotos en `premium-packs-haul.html` (exclusive/playlists/tokens/vr) → reemplazados; 1 mojibake cyrillic playlist link en `top-100-twerk-videos.html` → fixed. **(3) Legacy noindex** · 14 archivos legacy/test con `noindex,follow`: VARIANTE-A/B/showcase, oriental-final, paid-content, playlist-model-1-dark-premium, premium-* (8 files), savage-twerk-video. **(4) THE BIG ONE — playlist details mass noindex** · 319 archivos `/playlist/*.html` con descriptions template-duplicadas («[Title] with stronger internal links, cleaner navigation and curated video discovery inside Alexia Twerk Group»). Cambiados de `index,follow` a `noindex,follow`. **Razón:** Google no los estaba indexando igual (estaban en el bucket «Duplicate without user-selected canonical» 194 + «Discovered, not indexed» 171). El thin-content dañaba domain quality signals. Ahora el `/playlist/index.html` queda como canonical hub único, los 319 details siguen pasando link juice via `follow` a profiles/blog/creators. **Ojo:** la regla del HANDOFF original era no tocar URLs/slugs/paths — eso se respetó (solo se agregó meta tag, cero archivos eliminados). |
