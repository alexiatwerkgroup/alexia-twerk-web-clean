...
(function(){
  ...
  // â”€â”€ Inject ALL theater CSS at script load (NOT inside ensureModal — that
  ...
  function open(vid){
    ...
    loadYTApi().then(function(YT){
      ...
      ytPlayer = new YT.Player('twk-pl-theater-target', {
        ...
        events: {
          onReady: function(ev){
            // Set playback quality to 4K BEFORE playing (to avoid low-res buffering)
            try {
              var levels = ev.target.getAvailableQualityLevels();
              if (levels && levels.length > 0) {
                // Pick best quality: hd1440 (4K) → hd1080 → hd720 → medium
                ...
                // Use setPlaybackQuality (not Range) to force immediate quality
                ev.target.setPlaybackQuality(targetQuality);
              }
            } catch(_){}
            // NOW play after quality is set
  ...
  // â”€â”€ Time tracker — adds elapsed seconds to localStorage every 5s while playing
